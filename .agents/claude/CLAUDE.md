...
- Never start a `/loop` (autonomous/self-scheduled wakeup loop via `ScheduleWakeup`, or any recurring self-invocation) unless the user EXPLICITLY tells you to. Do NOT begin one on your own initiative, and do NOT reschedule one you weren't asked to start. They re-invoke you on a timer and burn usage. ALWAYS ask first.
